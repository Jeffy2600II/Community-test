...
// Behavior goal:
// - When dropdown is open and the user clicks anywhere outside, the first click
//   should only close the dropdown and NOT activate the element that was
//   clicked (no buttons should trigger, no :active/focus should be applied).
// - After that first click (which is consumed to close), normal clicks resume.
//
// Implementation:
// - Use a capture-phase pointerdown handler to intercept the user interaction
//   before page elements receive it. If outside the dropdown, preventDefault()
//   and stopImmediatePropagation(), then close dropdown and enable a short
//   "suppression window" during which click events (capture-phase) are also
//   prevented to guard against browser generating click from pointerup.
// - Keep the guards installed for a short grace period, then remove them so
//   normal clicks continue to work.

function addOutsideGuards() {
  // pointerdown capture: run before any target's mouse/pointer handlers or :active
  outsidePointerGuard = function (ev) {
    ...
    // Otherwise: prevent the pointer interaction from reaching the target,
    // close the dropdown, and start a short suppression window to also block
    // the subsequent click event (which may be synthesized on mouseup).
    ev.preventDefault();
    ...
    // enable short suppression so that any subsequent click generated by this
    // interaction doesn't activate the underlying element
    suppressUntil = Date.now() + 300; // 300ms suppression window
    if (suppressionTimeoutId) clearTimeout(suppressionTimeoutId);
    suppressionTimeoutId = setTimeout(() => {
      // After suppression window we remove guards
      removeOutsideGuards();
    }, 300);
  };

  // click capture: if within suppression window, prevent clicks too
  outsideClickGuard = function (ev) {
    ...
    // If suppression expired, we can remove guards proactively
    if (suppressionTimeoutId) {
      clearTimeout(suppressionTimeoutId);
      suppressionTimeoutId = null;
    }
    // Remove guards now because dropdown is closed and suppression expired
    removeOutsideGuards();
  };

  // key handler (Escape) to close dropdown
  outsideKeyGuard = function (ev) {
    if (ev.key === 'Escape' && dropdownVisible) {
      ev.preventDefault();
      hideDropdown();
      // after pressing Escape, remove guards immediately
      removeOutsideGuards();
    }
  };

  // add listeners in capture phase so they run before page handlers
...
function showDropdown() {
  ...
  // attach guards so the *next* click anywhere outside will only close the dropdown
  // and will not activate the underlying element
  addOutsideGuards();
}
function hideDropdown() {
  ...
  // Do NOT immediately remove guards here if suppression window is active:
  // addOutsideGuards scheduled removal of guards after suppression window.
  // If there is no suppression currently active, remove guards now.
  if (!suppressionTimeoutId) {
    removeOutsideGuards();
  }
}

// --- renderNav & rest of code --------------------------------------------
